...
def find_best_sim_aa(model, answers, id_article, articles_filter):
    ...
    if index_best_sim_ans <= 0:
        result += f"I can\'t find the best sim answer/answer. I take the answer in the last doc {answers[len(answers) - 1]}\
        at this link : <a href=\"https://huggy.insign.fr/post/{id_article[len(answers) - 1]}<\">https://huggy.insign.fr\
        /post/{id_article[len(answers) - 1]}</a><br>"
        result += highlight(articles_filter[len(answers) - 1], answers[len(answers) - 1])
    else:
        result += f"The link of the article <a href=\"https://huggy.insign.fr/post/{id_article[index_best_sim_ans]}\">\
        https://huggy.insign.fr/post/{id_article[index_best_sim_ans]}</a><br>So the answer is :<br>"
        result += highlight(articles_filter[index_best_sim_ans], answers[index_best_sim_ans])
    return result
